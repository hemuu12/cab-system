...
function ConfirmationContent() {
  ...
  // Next's router has no `location.hash` equivalent (it isn't tracked server or client side
  // as router state), so the access token is read directly from window.location.hash instead.
  const accessToken = typeof window !== 'undefined'
    ? new URLSearchParams(window.location.hash.replace(/^#/, '')).get('access') || ''
    : '';
  const { data: booking, error, isLoading } = useBookingQuery({ reference, accessToken }, { skip: !reference });

  if (error) return <div className="page-shell empty-state"><h1>Booking not found</h1><p>{errorMessage(error, 'We could not find this booking.')}</p><Link className="button button-gold" href="/">Return home</Link></div>;
  ...
  return <div className="page-shell confirmation"><div className="container narrow">
    ...
    {booking.passenger.email && <PremiumCard delay={.12} className="mt-5 border-champagne/30 bg-champagne/5 p-6 text-center">
      ...
      <Link className="button button-gold mt-5" href={otpLink}>Verify email and view trips</Link>
    </PremiumCard>}
    <div className="confirmation-actions"><Link className="button button-ghost" href={booking.passenger.email ? otpLink : '/login'}>View my trips</Link><Link className="button button-ember" href="/">Book another ride</Link></div>
  </div></div>;
}

export default function ConfirmationPage() {
  return <main><ConfirmationContent /></main>;
}
