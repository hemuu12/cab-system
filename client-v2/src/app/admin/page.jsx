...
import { ChevronLeft, ChevronRight, KeyRound, LogOut, Pencil, RefreshCw, Search, Trash2 } from 'lucide-react';
...
import { optimizedImageUrl } from '../../lib/images.js';
...
import { AdminTable, CreatePanel, Field, SelectField, Status, Toggle } from '../../components/admin/AdminControls.jsx';
import EditPanel from '../../components/admin/EditPanel.jsx';
import Overview from '../../components/admin/Overview.jsx';
import Pricing from '../../components/admin/Pricing.jsx';
import AdminSectionFilters, { EMPTY_ADMIN_FILTERS, filterAdminRows } from '../../components/admin/AdminSectionFilters.jsx';
...
} from '../../components/admin/constants.js';
...
const EDIT_TARGETS = {
  ...
  user: { resource: 'users', label: 'Administrator' }
};
...
function AdminContent() {
  ...
  const setSection = useCallback(nextSection => {
    ...
    router.replace(next === 'Overview' ? '/admin' : `/admin?section=${next.toLowerCase()}`);
  }, [availableSections, router]);
  ...
  const saveEdit = async event => {
    ...
    const userPassword = editing.type === 'user' ? readForm(form) : null;
    if (userPassword && userPassword.password !== userPassword.confirmPassword) {
      setError('The password confirmation does not match.');
      return;
    }
    const payload = editing.type === 'user'
    ...
    try {
      ...
      setEditing(null);
      toast.success(
        editing.type === 'user' ? `${item.email}'s password was changed and existing sessions were revoked.` : `${item.name || item.destination} was updated.`,
        `${label} saved`
      );
    ...
    <main className="admin-main">
      <header>
        ...
        <div className="admin-header-actions">
          <AdminNotifications user={user} onOpenSection={setSection} />
      ...
      {!isPricing && activeQuery.isLoading ? <AdminSkeleton section={section} /> : <AnimatePresence mode="wait">
        <motion.div key={section} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: .2 }}>
          ...
          {section === 'Users' && <><CreatePanel title="Create an administrator" subtitle="Only the super administrator can issue another admin ID and permanent password."><form className="admin-create-form admin-account-form" onSubmit={addAdmin}>
...
export default function Page() {
  return <Suspense fallback={<AdminSkeleton section="Overview" />}>
