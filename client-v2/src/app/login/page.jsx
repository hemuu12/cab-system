...
function LoginContent() {
  ...
  const requestedPath = fromParam.startsWith('/') && !fromParam.startsWith('//')
    ? fromParam
    : '';
  ...
  const completeSignIn = (session, title) => {
    ...
    router.replace(requestedPath || homeFor(session.user));
  };
    ...
    >
      ...
      <motion.form layout onSubmit={submit} className="login-form mt-5 grid gap-4">
        ...
        {otpMode && otpSent
          ...
          />}
        {!otpMode && !registering && <Link className="-mt-1 justify-self-end text-[11px] font-semibold text-champagne transition hover:text-ivory" href="/forgot-password">Forgot password?</Link>}
...
export default function Page() {
  return <Suspense fallback={<LoadingScreen message="Restoring your secure session…" detail="Keeping your journeys protected." />}>
    <LoginContent />
  </Suspense>;
}
