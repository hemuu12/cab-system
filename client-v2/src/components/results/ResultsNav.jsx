...
export default function ResultsNav({ user, onLogout }) {
  ...
  return <nav>
    <div className="wrap nav-in">
      <Link className="brand" href="/" aria-label="WonderTravel home">
      ...
      <div className="nav-cta">
        ...
        <button className="btn btn-gold" type="button" onClick={() => router.push('/#book')}>Book a ride</button>
      </div>
      <button
        ref={toggleRef}
      ...
      <div className={`mobile-menu${open ? ' open' : ''}`} id="mobileNav">
        ...
        {user ? <>
          ...
          <Link href="/account" onClick={close}>{firstNameOf(user.name)} · My profile &amp; trips</Link>
        </> : <Link className="mobile-account" href="/login" onClick={close}>Member login</Link>}
        {user && <button className="mobile-logout" type="button" onClick={() => { onLogout(); close(); }}>Log out</button>}
        <button className="mobile-primary" type="button" onClick={() => { close(); router.push('/#book'); }}>Book a ride</button>
      </div>
    </div>
  </nav>;
}
