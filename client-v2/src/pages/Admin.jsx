...
export default function Admin() {
  ...
  return <div className="admin-page">
    ...
    <main className="admin-main">
      <header>
        <div><StatusBadge>Live operations</StatusBadge><h1>{section}</h1><p>Manage WonderTravel from one secure workspace.</p></div>
      ...
      {activeQuery.isLoading ? <div className="loading admin-loading"><span/><p>Loading live data…</p></div> : <AnimatePresence mode="wait">
        <motion.div key={section} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: .2 }}>
          ...
          </form></CreatePanel><AdminTable heads={['Member', 'Admin ID / Email', 'Created', 'Role', 'Active']}>{rows.map(item => <tr key={item._id}><td><b>{item.name}</b><small>{item.createdFrom === 'admin' ? 'Created by admin' : 'Registered account'}</small></td><td>{item.email}</td><td>{longDate(item.createdAt)}</td><td><span className={`admin-role role-${item.role}`}>{item.role}</span></td><td><Toggle value={item.active} onChange={active => update(item._id, { active })} /></td></tr>)}</AdminTable></>}
        </motion.div>
      </AnimatePresence>}
    </main>
